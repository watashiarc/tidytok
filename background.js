...
import LinkOpener from './openLink.js';
...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  switch (info.menuItemId) {
    case "in1Hour":
      ...
      LinkOpener.scheduleOpen(tab.url, 3600000) // milliseconds in an hour
      break;

    case "tomorrow":
      handleContextMenuClick(tab, Time.calculateTomorrow());
      LinkOpener.scheduleOpen(tab.url, 86400000) // milliseconds in a day
      break;

    case "nextWeek":
      handleContextMenuClick(tab, Time.calculateNextWeek());
      LinkOpener.scheduleOpen(tab.url, 604800000) // milliseconds in a week
      break;
